...
class CcafcSpider(scrapy.Spider):
    ...
    start_urls = ['http://www.ccafc.org.cn/templates/DonationSearch/index.aspx?nodeid=106&name=&DonationNum=&MinAmount=&MaxAmount=&Payment=&starttime=2009-01-01&endtime=2019-03-13&Project=0&recipients=&type=%E4%BC%81%E4%B8%9A&pagesize=1&pagenum=20']
    def parse(self, response):
        ...
        if i < 2:
            sendmessage(
                '时间-{} 机构-{} 姓名-{}  数据数据量为0  url地址-{}'.format(datetime.datetime.now().strftime("%T-%m-%d %H:%M:%S"),
